...
1. **Extracts `efiboot.img`** using a three-tier fallback chain:
    ...
    3. **El Torito boot catalog** -- `osirrox -extract_boot_images` (falls back to `xorriso -osirrox on -extract_boot_images` if extraction fails) extracts hidden boot images from the boot ISO. Fedora 43 stores `efiboot.img` exclusively in the [El Torito](https://en.wikipedia.org/wiki/El_Torito_(CD-ROM_standard)) boot catalog and GPT appended partition, not as a visible ISO 9660 directory entry. El Torito is a standard for embedding hidden boot partitions in optical media -- these partitions are invisible to normal filesystem extraction (`osirrox -extract`) but contain the actual EFI FAT image that firmware reads at boot. The extracted files are first probed directly with `mcopy` to identify the one containing `grub.cfg` (checking both `/EFI/BOOT/grub.cfg` and `/EFI/fedora/grub.cfg`). If direct probing fails (e.g., the image has a GPT/MBR partition table wrapping the FAT filesystem), a two-stage fallback activates: **Stage 1** uses `sfdisk --json` + `jq` to detect partition table wrappers, locates the EFI System Partition (matching the type case-insensitively across GPT GUID, MBR type, and label), and extracts the raw FAT partition with `dd` using the correct sector size. **Stage 2** (if no partition table is found) scans for FAT boot sector signatures at bounded offsets within the first 4 MiB. The stripped image is then re-probed with `mcopy`. A post-selection sanity check verifies the result is a clean raw FAT image before assigning it to `$efi_img`. Ambiguous or empty results cause a hard failure with diagnostics including `file(1)` output and mcopy stderr.
